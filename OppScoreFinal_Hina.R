...
#
# Oringinally Written by JP Carrascal: www.jpcarrascal.com, www.github.com/jpcarrascal
# Modified by Hina Shah: 
#
# How to use:
...
# 4. Change the num_of_concepts variable so as to assign unique color code to each concept
#
######################################################################################################
...
# You can use other palettes from https://www.datanovia.com/en/blog/top-r-color-palettes-to-know-for-great-data-visualization/
library(RColorBrewer)
display.brewer.all()
display.brewer.pal(n = num_of_concepts, name = 'Dark2')
...
plotOppScore <- function(values)
{
  ...
  par(xpd=F)
  lowerLimit <- 0
  plot(NULL, xlim=c(lowerLimit,10), ylim=c(lowerLimit,10), yaxt="n", xaxt="n", xaxs="i", yaxs="i", xlab = "Importance", ylab = "Satisfaction", main = opp_landscape_title)
  ...
  par(xpd=T)
  points(values$importance, values$satisfaction, xlim=c(lowerLimit,10), ylim=c(lowerLimit,10), col= brewer.pal(n = 6, name = 'Dark2'), pch=19, cex=2)
  with(values, text(satisfaction~importance, labels = c(1:length(values$outcome)), pos = 2, col=1, font=2, srt=0, cex=0.8))
  legend(bg=rgb(0,0,0,0.0),lowerLimit+0.1,9.9, paste(c(1:length(values$outcome)),". ", values$outcome,
                                                     " (",round_df(values$oppscore,2),")",sep=""), text.font = 2, text.col = brewer.pal(n = 6, name = 'Dark2'), title = "CONCEPTS", title.adj = 0.1, bty = "n", cex=0.92, x.intersp=0.0, y.intersp=0.6)
}
...
#Hina - If you want aggregate, use this, otherwise use Median
...
imp <- aggregate(importance~outcome, inputData, median)
sat <- aggregate(satisfaction~outcome, inputData, median)
